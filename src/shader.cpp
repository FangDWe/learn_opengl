...
void ShaderProgma::use(){
    glUseProgram(ID);
}

void ShaderProgma::set_vec3(std::string name, float x, float y, float z) {
    glUniform3f(glGetUniformLocation(ID, name.c_str()), x, y, z);
}
void ShaderProgma::set_vec3(std::string name, glm::vec3 v) {
    glUniform3f(glGetUniformLocation(ID, name.c_str()), v.x, v.y, v.z);
}

void ShaderProgma::set_mat4(std::string name, glm::mat4 m){
    glUniformMatrix4fv(glGetUniformLocation(ID, name.c_str()), 1, GL_FALSE, glm::value_ptr(m));
}

void ShaderProgma::set_float(std::string name, float a){
    glUniform1f(glGetUniformLocation(ID, name.c_str()), a);
}

void ShaderProgma::set_int(std::string name, unsigned int a){
...
ShaderProgma::ShaderProgma(const char* vertex_shader_path, const char* frag_shader_path){
    ...
    if (!success)
    {
        ...
        std::cout << "Shader Progma Link Failed\n" << infoLog << std::endl;
    }
